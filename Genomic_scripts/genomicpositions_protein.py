...
# The second Input file is the unique protein regions file produced by the main pipeline
...
for line in uniquefile:
    ...
    id = elements[0].split(":")
    start =(int(elements[1]))# int(id[2]) + ((int(elements[1])))#
    end = (int(elements[2]))#int(id[2]) + ((int(elements[2])))#
    temp=[id[0],start,end,id[1],id[2],id[3]]
