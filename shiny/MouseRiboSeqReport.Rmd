...
The following document is a report on the alignment of multiple Ribo-Seq datasets against NMD and RI transcripts . These alignments are then analyzed in respect to their mapping against the previously determined unique Regions of the also previously determined split-ORFs.
...
setwd(params[[1]][[1]])
#setwd("G:/Justin_Backup_29.04.2020/Justin/Uni-Frankfurt/FP+Masterarbeit/PipeTest/Pipeline/shiny/splitORFdatabase/data/mouse/NMD/BOWTIE")
directories<-list.dirs(path = ".", full.names = TRUE, recursive = TRUE)

randomnmdfiles=list()
for(i in directories){
  if(!(identical(list.files(i,pattern="*NMD_mouse_random_3UTR_intersect_counts_relative_sorted.bed"), character(0)))){
    randomnmdfiles=c(randomnmdfiles,paste0(i,"/",list.files(i,pattern="*NMD_mouse_random_3UTR_intersect_counts_relative_sorted.bed")))
  }
}
...
for(i in directories){
  if(!(identical(list.files(i,pattern="*NMD_mouse_random_3UTR_intersect_counts_relative_sorted.bed"), character(0)))){
    randomSetnames=c(randomSetnames,list.files(i,pattern="*NMD_mouse_random_3UTR_intersect_counts_relative_sorted.bed"))
  }
}
randomnames=stringr::str_replace(randomSetnames, pattern = "_mouse_random_3UTR_intersect_counts_relative_sorted.bed", replacement = "")
...
setnames=list()
for(i in directories){
  if(!(identical(list.files(i,pattern="*NMD_mouse_intersect_counts_relative_sorted.bed"), character(0)))){
    nmdfiles=c(nmdfiles,paste0(i,"/",list.files(i,pattern="*NMD_mouse_intersect_counts_relative_sorted.bed")))
    setnames=c(setnames,list.files(i,pattern="*NMD_mouse_intersect_counts_relative_sorted.bed"))
  }
}
...
names(nmddataframes) <- stringr::str_replace(setnames, pattern = "_mouse_intersect_counts_relative_sorted.bed", replacement = "")
...
```{r, echo=FALSE,results = 'asis'}
setwd(params[[1]][[2]])
#setwd("G:/Justin_Backup_29.04.2020/Justin/Uni-Frankfurt/FP+Masterarbeit/PipeTest/Pipeline/shiny/splitORFdatabase/data/mouse/RI/BOWTIE")
ridirectories=list.dirs(path = ".", full.names = TRUE, recursive = TRUE)

randomrifiles=list()
for(i in ridirectories){
  if(!(identical(list.files(i,pattern="*RI_mouse_random_3UTR_intersect_counts_relative_sorted.bed"), character(0)))){
    randomrifiles=c(randomrifiles,paste0(i,"/",list.files(i,pattern="*RI_mouse_random_3UTR_intersect_counts_relative_sorted.bed")))
  }
}
...
for(i in ridirectories){
  if(!(identical(list.files(i,pattern="*RI_mouse_random_3UTR_intersect_counts_relative_sorted.bed"), character(0)))){
    randomRiSetnames=c(randomRiSetnames,list.files(i,pattern="*RI_mouse_random_3UTR_intersect_counts_relative_sorted.bed"))
  }
}
randomRinames=stringr::str_replace(randomRiSetnames, pattern = "_mouse_random_3UTR_intersect_counts_relative_sorted.bed", replacement = "")
...
risetnames=list()
for(i in ridirectories){
  if(!(identical(list.files(i,pattern="*RI_mouse_intersect_counts_relative_sorted.bed"), character(0)))){
    rifiles=c(rifiles,paste0(i,"/",list.files(i,pattern="*RI_mouse_intersect_counts_relative_sorted.bed")))
    risetnames=c(risetnames,list.files(i,pattern="*RI_mouse_intersect_counts_relative_sorted.bed"))
  }
}
...
ridataframes=rilist
names(ridataframes) <- stringr::str_replace(risetnames, pattern = "_mouse_intersect_counts_relative_sorted.bed", replacement = "")
