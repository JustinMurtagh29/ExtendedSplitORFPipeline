...
## Human Detained-Intron Transcripts 

First it provides an overview of the valid alignment results:


```{r basic stats,echo=FALSE}
#setwd("G:/Justin_Backup_29.04.2020/Justin/Uni-Frankfurt/FP+Masterarbeit/")
datapath=getwd()
path = "FinalAnalysis"

#read in all the Ensembl IDs with transcript support level annotation
validIds=read.table("G:/Justin_Backup_29.04.2020/Justin/Uni-Frankfurt/FP+Masterarbeit/Results/Detained_Introns/ValidProteinORFPairs.txt",header=T)#,fill=T,stringsAsFactors=F) 

tab = read.table("G:/Justin_Backup_29.04.2020/Justin/Uni-Frankfurt/FP+Masterarbeit/Results/Detained_Introns/UniqueProteinORFPairs_annotated.txt",header=T)#,fill=T,stringsAsFactors=F)

tab= subset(tab,tab$OrfTransID %in% validIds$OrfTransID)

NumTrans = nrow(validIds)
...
##
This is the histogramm of matching ORFs per transcripts:
...
##
This is the histogramm of NMD-protein pairs
```{r hist2, echo=FALSE}
...
These are the top 20 genes that have the most transcripts with >=2 ORFs
...
```
##
A histogramm of min and max sequence identity values from the ORF-protein alignments, i.e., each such value is derived from one ORF-protein alignment. For all ORFs of a protein-ORF match the min respectively max seq. identity is recorded. The minimal sequence identity allowed for an alignment by default is 90%.
...
##
To analyze the length of the predicted orfs in DNA basepairs we plot a histogram in the intervall [0,2000]. Note that the minimal length of an ORF is 120 base pairs, which explains the sudden break close to 0. In order to get an idea of av and analyze the mean lengthA histogramm of orf lengths.sequence min and max sequence identity values from the ORF-protein alignments. For all ORFs of a protein-ORF match the min respectively max seq. identity is recorded.
